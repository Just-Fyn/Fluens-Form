          ...
          {(q.options || []).map((o, oi) => {
            ...
            return (
              <label 
                key={o.id} 
                htmlFor={o.id}
                className={`relative flex items-center gap-4 p-4 rounded-3xl border border-border bg-foreground transition-all pointer-events-none 
      ...
      {q.type === "select" && (
        <>
          ...
          <div className={`w-full max-w-sm relative text-sm ${openSelect ? "z-50" : ""}`}>
            <button
              type="button"
              className="flex items-center justify-between px-6 w-full h-14.5 border border-border rounded-full cursor-default bg-brand-light/10 transition-all outline-none pointer-events-none"
            >
